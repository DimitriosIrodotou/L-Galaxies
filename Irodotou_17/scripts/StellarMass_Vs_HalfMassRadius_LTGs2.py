...
SHMRNoGS = np.load(SavePath + 'NoGS/' + 'StellarHalfMassRadius.npy')
...
Stellar_Mass_HWT15 = StellarMassHWT15[indexHWT15] * MassUnits

S_H_M_R_NoDI = SHMRNoDI[indexNoDI] * LengthUnits
Stellar_Mass_NoDI = StellarMassNoDI[indexNoDI] * MassUnits

S_H_M_R_NoGS = SHMRNoGS[indexNoGS] * LengthUnits
Stellar_Mass_NoGS = StellarMassNoGS[indexNoGS] * MassUnits
...
for i in range(nbin):
    ...
    if len(index) > 0:
        median[i] = np.nanmedian(S_H_M_R_NoDI[index])
        slow[i] = np.nanpercentile(S_H_M_R_NoDI[index], 15.87)
        shigh[i] = np.nanpercentile(S_H_M_R_NoDI[index], 84.13)
    log10XLow += dlog10
...
for i in range(nbin):
    ...
    if len(index) > 0:
        median[i] = np.nanmedian(S_H_M_R_NoGS[index])
        slow[i] = np.nanpercentile(S_H_M_R_NoGS[index], 15.87)
        shigh[i] = np.nanpercentile(S_H_M_R_NoGS[index], 84.13)
    log10XLow += dlog10

# Plot median and 1-sigma lines #
plt.plot(X, median, color='red', lw=lw, linestyle='dashdot', label=r'$\mathrm{This\; work: f=1.0}$')
