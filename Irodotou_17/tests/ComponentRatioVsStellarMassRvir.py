...
DistanceToCentralGal = np.empty(shape=(nGal, 3))
...
for iFile in range(firstFile, lastFile + 1):
    ...
    with h5py.File(outputDir + filePrefix + '%i' % iFile + filePostfix, 'r') as f:
        ...
        DistanceToCentralGal[iGal:iGal + nGalFile] = f[snap]['DistanceToCentralGal']
        iGal += nGalFile
...
Distance_To_CentralGal = np.linalg.norm(DistanceToCentralGal, axis=1) * LengthUnits

Distance_To_CentralGal = Distance_To_CentralGal[np.where(Distance_To_CentralGal  > 0.0)]
Disk_Mass = Disk_Mass[np.where(Distance_To_CentralGal  > 0.0)]
ID_Bulge_Mass = ID_Bulge_Mass[np.where(Distance_To_CentralGal  > 0.0)]
MD_Bulge_Mass = MD_Bulge_Mass[np.where(Distance_To_CentralGal  > 0.0)]
Stellar_Mass = Stellar_Mass[np.where(Distance_To_CentralGal  > 0.0)]
...
# Plot data for disks #
plt.scatter(Distance_To_CentralGal, Disk_Ratio, c='b', s=10, label="$\mathrm{Disk}$")
...
plt.scatter(Distance_To_CentralGal, ID_Ratio, c='g', s=10, label="$\mathrm{Pseudo\, bulges}$")
# plt.legend(loc=4, fancybox='True', shadow='True', fontsize=15, markerscale=1)
...
plt.scatter(Distance_To_CentralGal, MD_Ratio, c='r', s=10, label="$\mathrm{Classical\, bulges}$")
# plt.legend(loc=4, fancybox='True', shadow='True', fontsize=15, markerscale=1)
...
# Calculate median and 1-sigma for disks #
log10StellarMass = np.log10(Distance_To_CentralGal)
log10StellarMassMax = np.log10(max(Distance_To_CentralGal))
log10StellarMassMin = np.log10(min(Distance_To_CentralGal))
...
for i in range(nbin):
    ...
    mass[i] = np.mean(Distance_To_CentralGal[index])
    if len(index) > 0:
        median[i] = np.median(Disk_Ratio[index])
        slow[i] = np.percentile(Disk_Ratio[index], 15.87)
        shigh[i] = np.percentile(Disk_Ratio[index], 84.13)
...
# Calculate median and 1-sigma for classical bulges #
log10StellarMass1 = np.log10(Distance_To_CentralGal)
log10StellarMassMax1 = np.log10(max(Distance_To_CentralGal))
log10StellarMassMin1 = np.log10(min(Distance_To_CentralGal))
